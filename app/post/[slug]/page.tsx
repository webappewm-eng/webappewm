...
import { getPostBySlug, getPosts, saveFeedback, saveSubscription, trackAnalyticsEvent } from "@/lib/firebase/data";
...
export default function PostPage() {
  ...
  useEffect(() => {
    async function loadPost() {
      setLoading(true);
      const nextPost = await getPostBySlug(slug);
      setPost(nextPost);

      if (nextPost) {
        const allPosts = await getPosts({ categoryId: nextPost.categoryId });
        setRelated(allPosts.filter((item) => item.id !== nextPost.id).slice(0, 3));
      }
      setLoading(false);
    }
    ...
    <div className="app-shell">
      ...
      <main className="page-main">
        ...
        <div className="page-wrap">
          ...
          {loading ? <p className="muted">Loading post...</p> : null}
